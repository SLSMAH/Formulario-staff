...
const roleInfo = {
  ...
  admin: [
    ...
    "Inicio de sesión para Weis, Carlos y Kaizen. Cada cuenta utiliza control de IP pública."
  ]
};
...
form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (!response.ok) {
      const error = new Error(data.error || "Acceso denegado.");
    ...
    sessionStorage.setItem("cruza2_session", JSON.stringify({
      ...
      canApproveIp: Boolean(data.canApproveIp)
    }));
    ...
    if (error.code === "IP_APPROVAL_REQUIRED") {
      message.textContent =
        `${error.message} Solicitud #${error.requestId}. `
        + "Vuelve a intentarlo después de la aprobación.";
    } else {
      message.textContent = isNetworkError
        ? "No se pudo conectar con la API. Revisa la URL del Worker en config.js y la configuración CORS."
        : error.message;
    }
