...
function loadDraft() {
  try {
    ...
    const lockedChoices = saved.lockedChoices && typeof saved.lockedChoices === "object"
      ? saved.lockedChoices
      : {};

    for (const question of questions) {
      if (question.type === "choice" && answers[question.id]) {
        lockedChoices[question.id] = true;
      }
    }

    return {
      step: Number.isInteger(saved.step) ? saved.step : -1,
      discord: typeof saved.discord === "string" ? saved.discord : "",
      answers,
      lockedChoices
    };
  } catch (error) {
    console.warn("No se pudo recuperar el intento guardado.", error);
    return null;
  }
}
...
const state = {
  ...
  discord: savedDraft?.discord ?? "",
  answers: savedDraft?.answers ?? {},
  lockedChoices: savedDraft?.lockedChoices ?? {},
  sending: false,
  done: false
};
...
function saveDraft() {
  try {
    ...
    console.warn("No se pudo guardar el intento actual.", error);
  }
}
  ...
  content.innerHTML = `
    <div class="view">
      ...
      <div class="notice">
        ...
        La primera opción elegida queda bloqueada, incluso si vuelves atrás o recargas la página.
      </div>
...
function renderChoiceQuestion(question) {
  ...
  content.innerHTML = `
    <div class="view">
      <span class="question-type">Situación de roleplay · un solo intento</span>
      <h2 class="question-title">${escapeHtml(question.title)}</h2>
      <p class="question-helper">${escapeHtml(question.helper)}</p>

      <div class="options-grid ${isLocked ? "options-locked" : ""}">
      ...
      ${hasAnswered ? `
        <div class="answer-feedback ${isCorrect ? "correct" : "incorrect"}">
          ...
          <div>
            ...
            <p>
              ${isCorrect
                ? "Respuesta registrada. La opción quedó bloqueada y ya no puede modificarse."
                : "Respuesta registrada como incorrecta. Solo disponías de un intento y no puedes cambiarla."}
            </p>
          </div>
        </div>
      ` : `
